#!/usr/bin/env python3
"""
build_data.py — the Daggerheart corpus (titterpig-dsl-daggerheart/0.5) → data/*.js.

Everything the site shows comes from here; nothing is hand-typed. The shape is GENERIC and
hash-keyed — the engine reads it without knowing the game, and system/daggerheart/ interprets it:

    window.DAGGERHEART.index           { system, counts, books: [ … ] }                 data/index.js
    window.DAGGERHEART.records         [ {id, name, book, type, under, …} ]             data/records.js
    window.DAGGERHEART.books[<id>]     { id, label, kind, chapters: [ … ], entities: [root ids] }
    window.DAGGERHEART.entities[<h>]   { id, name, book, file, form, type, applies, parent, slot,
                                         children, desc, props, table, guidance, refs, rules, blocks }

The build is L5R5e's (sortilege-vtt-l5r5e, PLAN decision 4): a lossless shaped dump. Any DEF
that is not a property value is an ENTITY; DESCRIPTION, PROPERTIES, EXTENDS, APPLIES TO, RULES,
TABLE, GUIDANCE and REFERENCES are lifted to named fields only in exactly the spec's shape, and
every other node — ATTACK, FEATURES, POTENTIAL_ADVERSARIES, PLAYER_PRINCIPLES, ROLL_TABLE… — is
carried into `blocks`, in the corpus's order, for the system layer to read. An entity the corpus
leaves unhashed gets an id this build writes (`u:` + a digest of its file, its parent and its
name); the corpus is hashed so that there are none (PLAN H1).

Two books share one directory: *Hope & Fear*'s files carry the prefix `hope-and-fear-`, and every
other file is the Core Rulebook's. BOOKS is the only hand-written list in the build; every corpus
file must be claimed by exactly one book or this exits non-zero.

`.lore` is Markdown, not DSL: a lore chapter carries its text verbatim as one string, and
...
counts: each string at least as many times as the corpus prints it.
    ...
    python3 build/build_data.py [<path to titterpig-dsl-daggerheart/0.5>]
"""
import hashlib
import json
import os
...
DEFAULT_CORPUS = os.path.expanduser("~/Sortilege/Titterpig/DSL/titterpig-dsl-daggerheart/0.5")
FILE_PREFIX = "daggerheart-0.5-"

# ───────────────────────── the prefix → book map ─────────────────────────
#
# `label` is this build's own short name for the book (declared as ours to the gate); each
# chapter's own title is its file's NAME, verbatim. A file belongs to the book with the longest
# prefix it carries; the core's prefix is empty, so it takes every file no other book claims.
BOOKS = [
    {"id": "core", "label": "Core Rulebook", "kind": "book", "prefix": ""},
    {"id": "hope-and-fear", "label": "Hope & Fear", "kind": "book", "prefix": "hope-and-fear"},
]
...
BANNER = ("/* Generated by build/build_data.py from titterpig-dsl-daggerheart/0.5 — do not edit by hand.\n"
          "   Every string is verbatim from the DSL corpus; regenerate rather than patch. */\n")

REGISTER = """(function(){var d=%s;var T=window.DAGGERHEART=window.DAGGERHEART||{books:{},entities:{},loaded:{}};
...
def book_of(fn):
    ...
    if stem is None:
        return None
    hits = [b for b in BOOKS if not b["prefix"] or stem == b["prefix"] + os.path.splitext(stem)[1]
...
def main():
    ...
    index = {"system": "daggerheart", "books": index_books,
    ...
    with open(os.path.join(data_dir, "index.js"), "w", encoding="utf-8") as fh:
        fh.write(BANNER)
        fh.write("(function(){var T=window.DAGGERHEART=window.DAGGERHEART||{books:{},entities:{},loaded:{}};"
...
def records_of(entities):
    ...
    for h, e in entities.items():
        ...
        for p in e.get("props", []):
            ...
            elif p["name"] in REF_NAME_FIELDS and p.get("vk") == "ref" and (p.get("ref") or {}).get("name"):
                fields[p["name"]] = p["ref"]["name"]
