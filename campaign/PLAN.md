...
- **Open — for the owner:** standard 2 says the campaign's "behind the veil" document moves into the GM tabs, in the pack, and leaves the repo. Caul's is `campaign/docs/veil.html`, and on the public site it is served to anyone with the URL (`https://caul.sortilege.online/campaign/docs/veil.html` → 200, 124,676 bytes: *The Shape of the World*, *Setting Secrets — GM Canon*, the persons/places/factions/adversaries dossiers). The Notes-pane gate hides it on the GM page only. Portents' route: a deterministic converter into Overview / Places / People / Threads sections (`absorb_state.py`), an independent every-word check (`check_absorb.py`), then the file removed (it stays in git history; the repo is public).
