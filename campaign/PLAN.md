...
- **Enforce HTTPS on** (owner, 2026-09-24) — `http://` now 301-redirects to `https://caul.sortilege.online/`; https 200 (decision O3 satisfied).
- **Owner-side remaining:** the real **two-browser live session** proof (a player on a second device claims a character and rolls, the GM sees it) — the Worker + ops already passed the local round-trip.

## Family standards (owner, 2026-09-24; PLAYBOOK §4b) — from upstream `a1c5c6e`
- **Merged** `upstream/main` (`a1c5c6e`, the Daggerheart VTT's standards port): `robots.txt` + the robots meta on every page; the GM's material in the GM tabs (`engine/gm-text.js`, `engine/gm-panes.js`, Daggerheart's rebuilt Scenes / Threads / Encounters / People); the GM's ops local (never sent to the room); the fill-by-id seed; the `/gm/` gate; the book tabs off the public site. `engine/config.js` is `merge=ours`, so its two new keys were carried by hand with the kit's `port_config.py`: `siteBooks: false` and `gmGate`.
- **Caul's own fix:** the home tab's Age of Umbra callout linked to `#frames`, a book tab now hidden on the public site — the link shows only where the books are on (`campaign/site/site.js`, `booksShown()`).
- **Proof (browser on a fresh origin, localhost:8142, through the controls):** the public site with the books off → *Caul, Chronicle, The Company, Dramatis Personae, Factions, Atlas, Relics, Lore, Dice*, the callout without its link, robots meta, `robots.txt` 29 `User-agent` lines; `/gm/` → the gate, legible in Caul's theme → **Enter**; the seed filled **7** ids on a fresh browser (the S28 scene, 6 threads); Scenes: *Standards test scene* added with session *Session 29* and `OPEN` / `YOURS` drawn, **Mark played** → *Session 29 · 1 scene, played* folded; **Settings** → books on → the site adds *The books … Frames, Search* after the campaign's tabs and the callout's link returns; Notes → *Lift the Veil* → Behind the Veil (81,020 characters); `gm/`, `gm/vtt.html`, `gm/play.html`: **0 console errors**.
- **Worker redeployed** (`engine/ops.js` changed): see the commit after this one.
- **Behind the Veil moved into the GM tabs** (owner, 2026-09-24: "Agreed" — Portents' route, the text in the public seed, chosen over a private pack file after being told the seed is publicly served just as the document was). `campaign/source/absorb_veil.py` carried `campaign/docs/veil.html` (as at `d8e0fd7`) into `campaign/pack/seed.json` → `gm`: **Overview** — the cosmology (*The Shape of the World*, *Setting Secrets — GM Canon*), *The Faeries of Thistleglass*, the Whispering Deck vignette, the Author's Notes; **Places** — the 9 places of GM Canon — Places; **People** — the 9 persons, 9 factions and 3 adversaries, each dossier *about* the campaign layer's record of exactly its name where there is one (Borek, Kelun, Living Fortress, Sarru, Savel, Tamber, Ygva, Askavir — Mellon, Valghast, Guardian Beast, Stonewraith have none); **Behind the characters** — the Sylvie draft, about Sylvie Cerridwen. Each dossier's parts are its subsections; a third heading level is a bold line; the one table's rows are list lines (*cell · cell*). Not carried: the hub's cards and each article's masthead line and banner (they describe the document). **Gate:** `campaign/source/check_absorb.py` (regex reading of the HTML, independent of the converter) — *PASS: 9 articles, 11962 words — 0 failures*; `--plant` catches a changed word and a dropped subsection; the converter re-run is byte-identical. Removed: `campaign/docs/veil.html` (now 404), `build_veil.py`, `VttConfig.notes`; the Notes pane left out (`hidePanes: ['notes']`, its free notes are in Overview); the public home page's *Behind the Veil* card (it pointed at `#veil`, a GM-only document that no longer exists). **Browser (8142, a campaign seeded before the move — the live GM's case):** the reload filled `gm` — Overview 5, Places 9, People 21, Behind the characters 1 — and kept the GM's own scene; Borek's dossier in People, its *Relationships* unfolded with its links (to the public site's pages), and *About: Borek* → the Inspector shows the dossier above his statblock; all 254 text blocks render with no stray marks; Mellon removed in its editor stayed removed after a reload; 0 console errors. **Found and fixed on the way:** headings that carried bold in the source showed literal `**` as titles — headings are now plain. The old document stays in the public repo's history (`d8e0fd7` and earlier).
