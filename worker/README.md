# sortilege-vtt-daggerheart Worker
...
`engine/ops.js` (and `system/daggerheart/ops.js`) the browser uses, validates them by role (the GM
...
```bash
cd worker && npm install && npx wrangler dev --port 8794   # local; the app on localhost talks to it
