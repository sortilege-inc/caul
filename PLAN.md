...
## STOPPED HERE — M5 blocked

M5 (sessions with `wrangler dev` on 8794) needs a second dev server, and the preview tool allows
five per folder: four belong to other chats. `worker/node_modules` is installed (`npm ci`). To resume:
stop one of the other chats' servers (or ask for one to be stopped), then `preview_start
vtt-daggerheart-worker` and prove: the GM's *Start session*, a player at `http://127.0.0.1:8742/gm/play.html?s=CODE`
joins, claims, rolls on the phone panes; the GM's notes, arc, threads, encounters and `npcState`
absent from the player's state; a player's roll with Fear raises the GM's Fear.
